...
## Vul detail ##
In the router module `wireless`, user can setup wlan power parameters with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup wlan power, `wireless` module will save data to config file.
...
burp0_json['wireless']['wlan_host_2g']['power'] = '%s' % ('0' * int(1.5 * 1024 * 1024))
