...
## Vul detail ##
In the router module `wlan_access`, user can add wlan access white list with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup white list, `wlan_access` module will add data to config file.

![](normal_wlan_access_config_file.png)

If we send request with long value in specific json key, it will overflow and break the `wlan_access` module config file. 
