...
## Vul detail ##
In the router module `wireless`, user can setup ap isolate with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup ap isolate, `wireless` module will save data to config file.
...
burp0_json['wireless']['wlan_host_2g']['isolate'] = '%s' % ('A' * int(1.5 * 1024 * 1024))
