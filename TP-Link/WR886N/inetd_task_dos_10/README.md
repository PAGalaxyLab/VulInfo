...
## Vul detail ##
In the router module `ipMacBind`, user can setup ip mac binding with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup ip mac binding, `ipMacBind` module will save data to config file.

![](normal_config_file.png)

If we send request with long value in specific json key, it will overflow and break the `ipMacBind` module config file. 
...
burp0_json['ip_mac_bind']['name'] = '%s' % ('A' * int(1.5 * 1024 * 1024))
