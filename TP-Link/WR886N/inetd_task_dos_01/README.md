...
## Vul detail ##
In the router module `firewall`, user can setup dmz host with http post request. An normal request looks like below.

![](normal_post_request.png)

After setup dmz host, `firewall` module will store data to config file.
...
burp0_json['firewall']['dmz']['enable'] = '%s' % ('A' * int(1.5 * 1024 * 1024))
